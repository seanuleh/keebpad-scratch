...
static void hidd_event_callback(esp_hidd_cb_event_t event, esp_hidd_cb_param_t *param)
{
  switch(event) {
    case ESP_HIDD_EVENT_REG_FINISH: 
      ESP_LOGI("SEAN-HID-EVENT", "ESP_HIDD_EVENT_REG_FINISH!");
      if (param->init_finish.state == ESP_HIDD_INIT_OK) {
        ESP_LOGI("SEAN-HID-EVENT", "state === ESP_HIDD_INIT_OK");
    ...
    case ESP_HIDD_EVENT_DEINIT_FINISH: break;
      ...
      ESP_LOGI("SEAN-HID-EVENT", "it's connected!");
      ESP_LOGI("SEAN-HID-EVENT","hid_conn_id: %d", hid_conn_id);
      ESP_LOGI(LOG_TAG, "ESP_HIDD_EVENT_BLE_CONNECT");
      hid_conn_id = param->connect.conn_id;
      break;
    case ESP_HIDD_EVENT_BLE_DISCONNECT:
      sec_conn = false;
      ESP_LOGI("SEAN-HID-EVENT", "ESP_HIDD_EVENT_BLE_DISCONNECT");
      ESP_LOGI(LOG_TAG, "ESP_HIDD_EVENT_BLE_DISCONNECT");
      esp_ble_gap_start_advertising(&hidd_adv_params);
      break;
    case ESP_HIDD_EVENT_BLE_VENDOR_REPORT_WRITE_EVT:
      ESP_LOGI("SEAN-HID-EVENT", "ESP_HIDD_EVENT_BLE_VENDOR_REPORT_WRITE_EVT");
...
static void gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
  switch (event)
  {
    case ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT:
      ESP_LOGI("SEAN-GAP-EVENT", "ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT");
      esp_ble_gap_start_advertising(&hidd_adv_params);
      break;
    case ESP_GAP_BLE_SEC_REQ_EVT:
      ESP_LOGI("SEAN-GAP-EVENT", "ESP_GAP_BLE_ADV_DATA_SET_COMPLETE_EVT");
      for(int i = 0; i < ESP_BD_ADDR_LEN; i++) {
        ESP_LOGI(LOG_TAG, "%x:",param->ble_security.ble_req.bd_addr[i]);
      }
      esp_ble_gap_security_rsp(param->ble_security.ble_req.bd_addr, true);
      break;
    case ESP_GAP_BLE_AUTH_CMPL_EVT:
      ESP_LOGI("SEAN-GAP-EVENT", "ESP_GAP_BLE_AUTH_CMPL_EVT");
      sec_conn = true;
